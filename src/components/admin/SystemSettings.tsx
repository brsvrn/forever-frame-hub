...
export function SystemSettings({ adminEmail }: { adminEmail: string }) {
  ...
  if (loading) {
    return <div className="py-20 flex justify-center"><Loader2 className="w-8 h-8 animate-spin text-gold" /></div>;
  }
    ...
    <div className="animate-in fade-in duration-500 max-w-3xl">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-sm text-zinc-400">Platformun genel işleyişi ve varsayılan limitlerini buradan yapılandırın.</p>
        </div>
        <button 
          onClick={handleSave} 
          disabled={saving}
      ...
      <div className="space-y-6">
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6">
          ...
          <div className="flex items-center justify-between py-4 border-b border-zinc-800/50">
            <div>
              ...
              <p className="text-sm text-zinc-400 mt-1">Aktif edilirse siteye ziyaretçiler erişemez, 'Bakımda' sayfası görünür.</p>
            </div>
            <label className="relative inline-flex items-center cursor-pointer">
              <input type="checkbox" checked={settings.maintenance_mode} onChange={e => setSettings({...settings, maintenance_mode: e.target.checked})} className="sr-only peer" />
          ...
          <div className="flex items-center justify-between py-4">
            <div>
              ...
              <p className="text-sm text-zinc-400 mt-1">Platforma yeni kullanıcıların üye olmasını izin verir.</p>
            </div>
            <label className="relative inline-flex items-center cursor-pointer">
              <input type="checkbox" checked={settings.allow_new_registrations} onChange={e => setSettings({...settings, allow_new_registrations: e.target.checked})} className="sr-only peer" />
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <p className="text-xs text-zinc-500 mt-2">Yeni üye olan kullanıcılara otomatik tanımlanacak başlangıç pakedi.</p>
            </div>

            <div>
              <label className="block text-sm text-white mb-2">Maksimum Dosya Yükleme Boyutu (MB)</label>
              <input 
                type="number" 
                className="w-full bg-zinc-950 border border-zinc-800 rounded-lg px-4 py-2 text-zinc-300"
              ...
              />
              <p className="text-xs text-zinc-500 mt-2">Misafirlerin yükleyebileceği tek bir fotoğraf/videonun max MB sınırı.</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6">
          ...
          <div>
            ...
            />
            <p className="text-xs text-zinc-500 mt-2">Kullanıcı panellerinde "Yardım İsteyin" butonlarına tıklandığında açılacak adres.</p>
          </div>
        </div>
        ...
        {settings.maintenance_mode && (
          <div className="bg-rose-500/10 border border-rose-500/30 rounded-xl p-4 flex gap-3">
            ...
            <div>
              ...
              <p className="text-rose-500/70 text-xs mt-1">Eğer bu ayarı kaydederseniz, admin olmayan tüm kullanıcılar ve ziyaretçiler "Bakımdayız" sayfasına yönlendirilecektir.</p>
            </div>
          </div>
        )}
