...
import { Search, Download, Trash2, Heart, Filter, CheckSquare, Image as ImageIcon, Video, PlayCircle, Loader2 } from "lucide-react";
...
export function DashboardGallery({ invitation }: { invitation: InvitationRow }) {
  ...
  useEffect(() => {
    async function fetchUploads() {
      ...
      if (!error && data) {
        setUploads(data as GuestUpload[]);
      }
      setLoading(false);
    }
    ...
    const channel = supabase.channel('schema-db-changes')
      .on(
        ...
        (payload) => {
          setUploads((prev) => [payload.new as GuestUpload, ...prev]);
        }
      )
      .subscribe();
  ...
  const handleDelete = async () => {
    ...
    const confirmDelete = window.confirm(`${selectedIds.size} adet medyayı silmek istediğinize emin misiniz?`);
    ...
    for (const upload of uploadsToDelete) {
      if (upload.file_url) {
        try {
          const path = upload.file_url.split("guest_uploads/")[1];
          if (path) {
            await storage.deleteFile("guest_uploads", path);
          }
        } catch (err) {
  ...
  const toggleFavorite = async (e: React.MouseEvent, id: string, currentStatus: boolean) => {
    ...
    if (error) {
      ...
      setUploads(prev => prev.map(u => u.id === id ? { ...u, is_favorite: !currentStatus } : u));
    }
  };
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700 ease-out">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-zinc-400 text-sm">Misafirlerinizin paylaştığı orijinal fotoğraflar ve videolar.</p>
        </div>
        ...
        <div className="flex items-center gap-2">
          {selectedIds.size > 0 && (
            <div className="flex items-center gap-2 mr-4 bg-gold/10 px-4 py-2 rounded-lg border border-gold/20">
              ...
              <button className="p-2 text-zinc-300 hover:text-white bg-zinc-800 rounded-md transition-colors tooltip" title="ZIP olarak İndir">
          ...
          <button onClick={toggleSelectAll} className="p-2.5 bg-zinc-900 border border-zinc-800 text-zinc-400 hover:text-white rounded-xl transition-colors" title="Tümünü Seç">
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        ...
        <div className="flex bg-zinc-900/50 border border-zinc-800 rounded-xl p-1 shrink-0">
          {[
            ...
            >
              {f.icon && <f.icon className={`w-4 h-4 ${f.id === "favorites" ? "text-rose-500" : ""}`} />}
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4">
        {filteredUploads.map(upload => {
          ...
          return (
            ...
            >
              <img 
                src={`${upload.file_url}?auto=format&fit=crop&w=400&q=60`} // Thumbnail generation hint
                alt="" 
              ...
              {/* Selection Indicator */}
              <div className={`absolute top-3 left-3 w-6 h-6 rounded-full border-2 flex items-center justify-center transition-colors ${isSelected ? "bg-gold border-gold text-black" : "border-white/50 bg-black/20 opacity-0 group-hover:opacity-100"}`}>
              ...
              <div className="absolute bottom-0 left-0 right-0 p-4 translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-300">
                ...
                <div className="flex items-center justify-between mt-1">
                  <p className="text-zinc-400 text-xs">{(upload.file_size / (1024*1024)).toFixed(1)} MB</p>
                  <button className="text-gold text-xs font-medium hover:underline">İndir</button>
                </div>
              </div>
            </div>
          );
