    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700 ease-out">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        ...
        <button
          type="button"
          onClick={downloadCsv}
          className="flex items-center gap-2 px-4 py-2 bg-zinc-900 border border-zinc-800 text-zinc-300 hover:text-white rounded-xl transition-colors text-sm"
        >
            ...
            <tbody className="divide-y divide-zinc-800/50">
              {filteredRsvps.map((rsvp) => (
                <tr key={rsvp.id} className="hover:bg-zinc-800/20 transition-colors">
                  ...
                  <td className="px-6 py-4 hidden md:table-cell text-zinc-400 max-w-xs truncate">
                    {rsvp.note || "-"}
                  </td>
