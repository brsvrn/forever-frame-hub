...
export function PremiumQRExperience({
  ...
  const mutedTextClass = isLueur ? "text-[#526176]" : "text-white/70";
  const optionClass = isLueur
    ? "border-[#b79a5d]/30 bg-[#07152f]/[.04] hover:bg-[#07152f]/[.08]"
    : "border-white/10 bg-white/5 hover:bg-white/10";
  const optionTextClass = isLueur ? "text-[#07152f]" : "text-white";
  ...
  return (
    <section className="relative py-24 px-6 flex flex-col items-center snap-center">
      ...
      >
        ...
        <h3 className={`text-3xl mb-4 ${theme.styles.typography.display}`}>Anıları Paylaşın</h3>
        <p className={`${mutedTextClass} text-sm mb-10 leading-relaxed`}>
        ...
        <div className="grid grid-cols-2 gap-4">
          <GuestUploadForm theme={theme} invitationId={invitationId}>
            <div
              className={`p-6 rounded-2xl flex flex-col items-center justify-center gap-3 transition-all border h-full w-full ${optionClass}`}
            >
              <Camera className={`w-6 h-6 ${optionTextClass}`} />
              <span className={`${optionTextClass} font-medium text-sm`}>Kamera</span>
            </div>
          </GuestUploadForm>
          ...
          <GuestUploadForm theme={theme} invitationId={invitationId}>
            <div
              className={`p-6 rounded-2xl flex flex-col items-center justify-center gap-3 transition-all border h-full w-full ${optionClass}`}
            >
              <ImagePlus className={`w-6 h-6 ${optionTextClass}`} />
              <span className={`${optionTextClass} font-medium text-sm`}>Galeri</span>
            </div>
          </GuestUploadForm>
        </div>
        ...
        <div
          className={`flex items-center justify-center gap-2 text-sm mt-10 ${
            isLueur ? "text-[#647086]" : "text-white/40"
          }`}
        >
