  ...
  return (
    <section className="relative flex min-h-dvh snap-center flex-col items-center px-6 py-24">
      <div className="w-full max-w-3xl">
        <h3 className={`mb-10 text-center text-2xl sm:text-3xl font-sans font-bold tracking-tight ${theme.styles.textColor || "text-white"}`}>
        ...
        <div className="space-y-5">
          {schedules.map((schedule) => (
            ...
            >
              <h4 className={`font-sans font-bold text-xl sm:text-2xl tracking-tight ${theme.styles.textColor || "text-white"}`}>
                {schedule.title}
              </h4>
              {schedule.description ? (
                <p className={`mt-2 font-sans text-sm leading-relaxed ${theme.styles.mutedTextColor || "text-white/75"}`}>{schedule.description}</p>
              ) : null}
              <div className={`mt-6 grid gap-4 font-sans text-sm font-medium ${theme.styles.textColor || "text-white/90"} sm:grid-cols-3`}>
                ...
                <p className="flex items-start gap-2">
                  ...
                  <span className="font-sans">
                    <strong className="font-semibold block">{schedule.venue_name}</strong>
                    {schedule.address ? (
                      <span className={`mt-1 block font-sans text-xs ${theme.styles.mutedTextColor || "text-white/70"}`}>{schedule.address}</span>
                    ) : null}
                  </span>
                </p>
              </div>
              {schedule.dress_code || schedule.parking_info || schedule.transport_info ? (
                <div className={`mt-5 rounded-2xl border border-white/10 bg-white/5 p-4 font-sans text-xs leading-relaxed ${theme.styles.mutedTextColor || "text-white/75"}`}>
                  {[schedule.dress_code, schedule.parking_info, schedule.transport_info]
                    .filter(Boolean)
                    .join(" · ")}
                </div>
