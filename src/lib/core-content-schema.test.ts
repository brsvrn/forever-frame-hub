...
import { memorySettingsSchema, rsvpSettingsSchema } from "./core-content-schema";
...
describe("core content validation", () => {
  ...
  it("normalizes empty upload dates to null", () => {
    const result = memorySettingsSchema.parse({
      photo_enabled: true,
      video_enabled: true,
      text_note_enabled: true,
      audio_message_enabled: false,
    ...
    expect(result.upload_starts_at).toBeNull();
    expect(result.upload_ends_at).toBeNull();
  });

  it("normalizes an empty RSVP deadline to null", () => {
    const result = rsvpSettingsSchema.parse({
      is_enabled: true,
      collect_phone: true,
      collect_email: false,
    ...
    expect(result.response_deadline).toBeNull();
  });
